...
import { DateInput, DateTimeInput, TextareaInput, NumberInput, ToggleSwitch, MultiSelectInput } from "./inputs";

const CONTROLLED_COMPONENTS = new Set(["DateInput", "DateTimeInput", "TextareaInput", "NumberInput", "ToggleSwitch", "MultiSelectInput"]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={cn("space-y-4", className)}>
      {Object.entries(columns).map(([colName, colDef]: [string, any]) => {
        ...
        // 5. Renderizar campos adicionais controlados (data, textarea, número, switch, multiselect)
        if (CONTROLLED_COMPONENTS.has(componentType)) {
          return (
            <div key={colName} className="flex flex-col gap-1">
              <label className="text-sm font-medium">{labelText}</label>
              <Controller
                ...
                control={control}
                render={({ field }) => {
                  const commonProps = { value: field.value, onChange: field.onChange, disabled: isDisabled, error: fieldError };
                  switch (componentType) {
                    case "DateInput":
                      return <DateInput {...commonProps} />;
                    case "DateTimeInput":
                      return <DateTimeInput {...commonProps} timezone={colDef.metadata?.ui_control?.timezone} />;
                    case "TextareaInput":
                      return (
                        <TextareaInput
                          {...commonProps}
                          maxLength={colDef.metadata?.validation?.maxLength}
                          minLength={colDef.metadata?.validation?.minLength}
                        />
                      );
                    case "NumberInput":
                      return (
                        <NumberInput {...commonProps} min={colDef.metadata?.validation?.min} max={colDef.metadata?.validation?.max} />
                      );
                    case "ToggleSwitch":
                      return <ToggleSwitch {...commonProps} label={labelText} />;
                    case "MultiSelectInput":
                      return (
                        <MultiSelectInput
                          {...commonProps}
                          options={(colDef.metadata?.validation?.options || []).map((opt: string) => ({ value: opt, label: opt }))}
                        />
                      );
                    default:
                      return <></>;
        ...
        // 6. Renderização Headless Fallback dos Componentes Nativos
        return (
          <div key={colName} className="flex flex-col gap-1">
            <label className="text-sm font-medium">{labelText}</label>
