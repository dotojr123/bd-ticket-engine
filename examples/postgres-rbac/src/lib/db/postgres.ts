// Importado só como TIPO — `pg` é carregado via require() dentro de extractPostgresSchema(),
// para não forçar essa dependência nativa em quem só usa o driver SQLite (ver mesmo padrão em
// src/lib/runtime/db-client.ts).
...
export async function extractPostgresSchema(connectionString: string): Promise<{ [tableName: string]: TableDefinition }> {
  ...
  const client: PgClient = new Client({ connectionString });
