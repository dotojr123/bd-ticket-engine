...
program
  ...
  .action(async (options) => {
    ...
    console.log(cli.info(`Iniciando extração com o driver: '${driver}'...`));

    try {
      if (driver === "postgres") {
        const url = options.url || process.env.DATABASE_URL;
        if (!url) {
          console.error(cli.error("Não encontramos DATABASE_URL configurado."));
          console.error(cli.fix("Defina DATABASE_URL no seu .env (veja .env.example), ou passe a connection string diretamente via --url \"postgres://usuario:senha@host:5432/banco\"."));
          console.error(cli.fix("Se o banco é local/SQLite, use --driver sqlite --d1-binding <arquivo.db> em vez de postgres."));
          process.exit(1);
        }
        ...
        if (!fs.existsSync(dbPath)) {
          console.error(cli.error(`Arquivo de banco SQLite não encontrado em '${cli.fileLink(dbPath)}'.`));
          console.error(cli.fix(`Crie o banco nesse caminho, ou aponte para o arquivo certo via --d1-binding <caminho>.`));
          process.exit(1);
        }

        console.log(cli.info(`Lendo banco SQLite/D1 em: ${cli.fileLink(dbPath)}`));
        console.log(cli.info(`Mesclando configuração fallback de: ${cli.fileLink(configPath)}`));
      ...
      if (tableCount === 0) {
        console.warn(cli.warn("Nenhuma tabela encontrada no banco."));
        console.warn(cli.fix("Se você esperava tabelas aqui, confira se está apontando para o banco certo e se as tabelas já existem (rode alguma migração antes, se aplicável)."));
      }

      if (options.dryRun) {
        console.log(cli.info(`[DRY-RUN] ${tableCount} tabela(s) e ${columnCount} coluna(s) seriam extraídas. Nenhum arquivo foi escrito em disco.`));
      ...
      console.log(cli.success(`Extração concluída com sucesso! Contrato gerado em: ${cli.fileLink(outputPath)}`));
      console.log(cli.dim(`  Próximo passo: npm run db:codegen`));
      logMetric("extract", { driver, tableCount, columnCount, durationMs: Date.now() - startedAt, dryRun: false });
    } catch (e: any) {
      console.error(cli.error(`Ocorreu uma falha crítica na extração:\n  => ${e.message}`));
      process.exit(1);
    }
  });
