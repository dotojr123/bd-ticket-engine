...
const utils_1 = require("../src/lib/ui/utils");
// Mock da função interna de resolução de visibilidade/permissão do DynamicForm
function resolvePermissions(colDef, userRole) {
...
describe("Dynamic UI Helpers and Logic Suite", () => {
    test("T004: cn utility should merge tailwind classes and resolve conflicts", () => {
        ...
        expect((0, utils_1.cn)("px-2 py-1", "bg-red-500")).toBe("px-2 py-1 bg-red-500");
        // Resolver conflitos de padding do Tailwind
        expect((0, utils_1.cn)("p-4", "p-2")).toBe("p-2");
        // Condicionais
        expect((0, utils_1.cn)("px-2", false && "bg-blue-500", "text-white")).toBe("px-2 text-white");
    });
