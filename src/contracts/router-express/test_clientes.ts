...
export const test_clientesRouter = Router();
...
const rbacWrite = requireRoleExpress(["admin"]);
const rbacRead = requireRoleExpress(["admin","user","parceiro"]);

test_clientesRouter.use(rateLimitExpress());

test_clientesRouter.get("/", rbacRead, async (req, res) => {
...
test_clientesRouter.get("/:id", rbacRead, async (req, res) => {
  const db = getDbClient();
  const user = req.user;
...
test_clientesRouter.post("/", rbacWrite, async (req, res) => {
  const db = getDbClient();
  const user = req.user;
  const parsed = test_clientesInsertSchema.safeParse(req.body);
...
test_clientesRouter.put("/:id", rbacWrite, async (req, res) => {
  const db = getDbClient();
  const user = req.user;
  const parsed = test_clientesUpdateSchema.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  try {
...
test_clientesRouter.delete("/:id", rbacWrite, async (req, res) => {
  const db = getDbClient();
  const user = req.user;
  try {
